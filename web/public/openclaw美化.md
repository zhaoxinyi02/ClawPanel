...
<img src="/logo.jpg" width="720" style="border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);"/>

# 🎛️ ClawPanel · OpenClaw 智能管理面板

**单文件部署 | 跨平台支持 | 全功能可视化管理 | 多智能体控制台**

Go 单二进制 · React 18 · TailwindCSS · SQLite · WebSocket 实时推送 · 全平台兼容

---

<div align="center">
  <img src="https://img.shields.io/badge/license-CC%20BY--NC--SA%204.0-red?style=for-the-badge" alt="License"/>
  <img src="https://img.shields.io/badge/version-5.2.1-violet?style=for-the-badge" alt="Version"/>
  <img src="https://img.shields.io/badge/go-1.22+-00ADD8?style=for-the-badge&logo=go&logoColor=white" alt="Go"/>
  <img src="https://img.shields.io/badge/react-18-61DAFB?style=for-the-badge&logo=react&logoColor=white" alt="React"/>
</div>

<div align="center">
  <img src="https://github.com/zhaoxinyi02/ClawPanel/actions/workflows/ci.yml/badge.svg?style=for-the-badge" alt="CI"/>
  <img src="https://github.com/zhaoxinyi02/ClawPanel/actions/workflows/release.yml/badge.svg?style=for-the-badge" alt="Release Build"/>
  <img src="https://img.shields.io/github/stars/zhaoxinyi02/ClawPanel?style=for-the-badge&logo=github" alt="GitHub Stars"/>
</div>

---

[✨ 快速开始](#-快速开始) · [🚀 功能特性](#-核心功能) · [📝 更新日志](#-更新日志) · [📚 API 文档](docs/API.md) · [🇺🇸 English](README_EN.md)
...
> 📱 **扫码加入企微群**
>
> 当前内置帮助页未打包企微群二维码图片，如需加入请前往项目仓库查看最新群二维码说明。
...
| 功能特性 | 详细说明 |
|:---|:---|
| 📋 工作流管理 | 工作流设置、模板 CRUD、AI 生成模板、运行列表、步骤详情、事件流与删除实例 |
| 🔄 会话接管 | 支持复杂任务自动接管原会话：即时确认、进度回写、暂停 / 恢复 / 重试 / 审批 / 继续 |
| 🎯 节点类型 | 节点支持 `input / wait_user / approval / ai_plan / ai_task / analyze / summary / publish / end` |
| 🛠️ 技能集成 | 节点支持 `skill` 参数，可自动加载 `SKILL.md` 参与执行 |
| 📁 文件产出 | 关键步骤结果自动落文件到 OpenClaw 工作区，并在工作流详情页显示产出文件、回传状态、失败原因 |
| 📤 自动回传 | QQ 私聊 / 群聊支持工作流完成后自动回传最终文件，支持预览、下载、单文件重发与批量重发最终文件 |
...
| 功能特性 | 详细说明 |
|:---|:---|
| 📊 Agent 管理 | Agent 列表管理：新建 / 编辑 / 删除，支持默认 Agent 设置 |
| 📄 Core Files | 可直接查看和保存 Agent 工作区核心文件 |
| 🔧 资源概览 | `Skills · Channels · Cron`：从单 Agent 视角查看技能、通道和定时任务快照 |
| 📅 会话巡检 | `Recent Sessions`：快速巡检当前 Agent 最近活跃会话 |
| 🛣️ 路由管理 | Bindings 路由规则管理：支持**结构化表单 + JSON 高级模式**，可启停、重排和规则级错误定位 |
| 🔍 路由预览 | 输入 `channel/sender/peer/parentPeer/guildId/teamId/accountId/roles` 快速验证命中 Agent |
...
| 🔧 兼容性修复 | 自动为非 OpenAI 提供商注入 `compat.supportsDeveloperRole=false` 兼容性修复 |
| 💾 自动备份 | `openclaw.json` 写入前自动快照备份（`backups/pre-edit-*.json`） |
...
| ⚙️ 可视化配置 | 自动读取插件 `plugin.schema.json`，前端动态生成配置表单 |
| 📋 插件日志 | 实时查看插件运行日志，便于调试 |
| 📦 多来源安装 | 支持从官方仓库、GitHub/Gitee 仓库、本地目录安装 |
| 📚 开发文档 | 详细的 [插件开发指南](docs/plugin-dev/README.md)、JSON Schema 规范、示例插件 |
| 🛡️ 冲突检测 | 安装前检查 ID、端口、依赖冲突 |
...
| 功能特性 | 详细说明 |
|:---|:---|
| 🔍 状态检测 | 每 30 秒检测 NapCat 连接状态（容器/WS/HTTP/QQ 登录） |
| 🔄 自动重连 | 连续离线自动重启（Docker 或 Windows 原生进程），可配置重连次数上限 |
| 🪟 Windows 原生支持 | Windows 用户安装 NapCat 时自动使用 Shell 版（无需 Docker） |
| 📊 状态面板 | 通道管理页面实时状态面板：在线/重连中/登录失效/离线指示灯 |
| 🔧 手动操作 | 手动重连按钮 + 重连日志查看 |
| ⚡ 实时推送 | 状态变化通过 WebSocket 实时推送 |
...
| 功能特性 | 详细说明 |
|:---|:---|
| 🔍 环境检测 | 系统环境检测（OS / CPU / Go / OpenClaw 版本） |
| 💾 备份恢复 | 配置备份与恢复（自动备份当前配置再恢复） |
| 📦 软件中心 | 软件安装中心：一键安装 Docker、NapCat、微信机器人等 |
| 📨 消息中心 | 安装任务进度实时追踪 |
| 📄 文档编辑 | 身份文档编辑（IDENTITY.md / USER.md 等） |
| 🔐 安全管理 | 管理密码修改、版本更新检查 |
...
| 功能特性 | 详细说明 |
|:---|:---|
| 🚀 国内加速 | 基于北京服务器国内加速，解决 GitHub 下载不稳定问题 |
| 🔄 全自动化 | 检查更新 → 下载 → SHA256 校验 → 替换程序 → 自动重启，全自动化 |
| 🛡️ 独立更新 | 更新进程通过 `systemd-run --scope` 隔离运行，主进程停止后更新不中断 |
| 📢 结果通知 | 更新成功后弹窗展示版本号和更新内容 |
| 💾 自动备份 | 自动备份旧程序（`.bak`），SHA256 校验防损坏 |
| 📦 离线更新 | 支持离线更新：上传本地二进制文件直接替换 |
...
- **执行步骤**：
  1. `go vet ./...`
  2. `go test -count=1 -shuffle=on ./...`（`ubuntu/windows` 矩阵）
  3. `go test -race -covermode=atomic -coverprofile=coverage.out ./...`
  4. `web npm ci + npm run build`
  5. 后端嵌入前端产物构建（`make backend-only`）
- **产出物**：
  - `go-coverage`（`coverage.out` + `coverage.txt`）
  - `frontend-dist`
  - `clawpanel-linux-amd64-ci`（用于快速验收）

### Release Build 工作流（`.github/workflows/release.yml`）

- **触发条件**：`push tag v*`（如 `v5.2.1`）/ 手动触发
- **执行步骤**：自动构建 `linux/darwin/windows` 多平台二进制 + `ClawPanel-Setup-v{version}.exe`
- **发布流程**：tag 触发时自动上传到 GitHub Release，并生成 `checksums.txt`
...
<p align="center">
  <a href="https://github.com/zhaoxinyi02"><img src="https://avatars.githubusercontent.com/u/98445030?v=4" width="64" height="64" alt="zhaoxinyi02" style="border-radius: 50%; margin: 0 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"/></a>
  <a href="https://github.com/BlueSkyXN"><img src="https://avatars.githubusercontent.com/u/63384277?v=4" width="64" height="64" alt="BlueSkyXN" style="border-radius: 50%; margin: 0 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"/></a>
  <a href="https://github.com/Hns16"><img src="https://avatars.githubusercontent.com/u/192765150?v=4" width="64" height="64" alt="Hns16" style="border-radius: 50%; margin: 0 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"/></a>
  <a href="https://github.com/codeKing6412"><img src="https://avatars.githubusercontent.com/u/185812512?v=4" width="64" height="64" alt="codeKing6412" style="border-radius: 50%; margin: 0 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"/></a>
</p>
...
<div align="center">
  <sub>Built with ❤️ by the ClawPanel Team</sub>
</div>